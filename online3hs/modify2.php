...
switch ($REQUEST_METHOD) {
	case "GET":
		...
		if(empty($data)) WebApp::redirect("/?act=online3hs.write2&mcode=$code&num=$num","아직 보고서를 작성하지 않았습니다.");
		...
		if (getenv('REMOTE_ADDR') == '125.130.18.5') {
			$tpl->define("CONTENT","/html/online3hs/modify2_adm.htm");
		} else {
			$tpl->define("CONTENT","/html/online3hs/modify2.htm");
		}
		
		//첨부파일 가져오기
		$tpl->define("FILE","CONTENT");
		$sql = "select * from $tb_online3hs_files where num_oid='$oid' and num_mcode='$code' and num_main='$num' $select_where";
		...
		$acc_link_val = "/?act=online3hs.acc_stu2&mcode=$code&num=$num";
		$acc_title_val = "승인";
		if($_SESSION[acc_tmp1] == 7){//담당교사
			...
			if($data[str_acc_tmp5] == "Y"){//학부모가 승인을 했으면 수정이 불가능하게 하자
				...
				$tpl->assign(array("acc_link"=> "/?act=online3hs.modify2&mcode=$code&num=$num", "acc_title"=>"수정"));
			}
	...
	case "POST":
		...
		//insert values
		$sql = "update $tb_online3hs_bogo set	$update_set	where num_oid='$oid' and num_mcode='$code' and num_serial='$num' and str_id='$bid'";
		//echo $sql; exit;
		//file 처리
		
		if($_FILES){
			$pathdir = _DIR_MAIN.'/hosts/'.$HOST.'/files/online3hs';
			if(!is_dir( $pathdir)) mkdir($pathdir,0777);
			
			$pathdir_y = _DIR_MAIN.'/hosts/'.$HOST.'/files/online3hs/'.$str_year;
			if(!is_dir( $pathdir_y)) mkdir($pathdir_y,0777);
			
			$pathdir_s = _DIR_MAIN.'/hosts/'.$HOST.'/files/online3hs/'.$str_year."/".$bid;
			if(!is_dir($pathdir_s)) mkdir($pathdir_s,0777);
			
			$pathdir_d = _DIR_MAIN.'/hosts/'.$HOST.'/files/online3hs/'.$str_year."/".$bid."/".$num;
			if(!is_dir($pathdir_d)) mkdir($pathdir_d,0777);
			
			$updirpath = "files/online3/$str_year/$bid/$num";
			$file_sql="select max(num_serial) from $tb_online3hs_files where num_oid=".$oid." and num_mcode=".$code." and num_main=".$num;
			$file_serial = $BoardDB->sqlFetchOne($file_sql);
			$file_serial = ($file_serial) ? $file_serial+1 : 1;
			$cnt = count($_FILES['upfiles']['name']);
			
			$del_file = "delete from $tb_online3hs_files where num_oid='$oid' and num_mcode='$code' and num_main='$num' and str_id='$bid'";
			...
			for($i=0;$i<$cnt;$i++){
				...
				if($file_name){
					...
					$up_file_name = "files/online3hs/$str_year/$bid/$num/".$up_file_name_s;
					...
					if(is_uploaded_file($file_tmp_name) && ($file_size>0)){
						...
						if(move_uploaded_file($file_tmp_name, $destination)){
							$file_sql="insert into $tb_online3hs_files values ('$oid','$code','$num','$file_serial','$str_name','$bid','$str_id','$str_partcode','$re_file_name','$up_file_name','$ip',SYSDATE,'$str_year','$up_file_name_s')";
...
function cb_format_list(&$arr){
	...
	$arr['download'] = "/?act=online3hs.download&mcode=$code&num=$num&bid=".$arr['str_id']."&idx=".$arr['num_serial'];
