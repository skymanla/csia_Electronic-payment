...
if(is_array($_SESSION[CAFE_MEM_TYPE])){
	foreach($_SESSION[CAFE_MEM_TYPE] as $key => $val){
		if($school_year == substr($key, 0, 4)){//권한 변경에 의해 해당 학년에 학급이 없는 경우 담임 세션은 없앤다
			$str_partcode = $key;
		}else{
			$str_partcode = "";
		}
...
if($_SESSION['acc_tmp1'] || $_SESSION['acc_tmp3'] || $_SESSION['acc_tmp4'] || $_SESSION[ADMIN]){
	if($report_sess == true){
		$sql = "SELECT COUNT(*) FROM 
				$tb_online3hs a left join $tb_online3hs_bogo b 
		...
		$sql = "SELECT COUNT(*) FROM $tb_online3hs WHERE num_oid='$oid' and num_mcode='$mcode' $search_phase $search_idx";
	}
	...
	if($report_sess==true){
		...
		$sql = "SELECT COUNT(*) FROM $tb_online3hs WHERE num_oid='$oid' and num_mcode='$mcode' and str_id='$str_id' and str_loginid='$str_loginid' $search_phase $search_idx";
	}
...
if($report_sess==true){
		$sql = "SELECT * FROM (
			SELECT /*+ INDEX_DESC (TAB_ONLINE3 IDX_TAB_ONLINE3) */
			  ...
			  FROM $tb_online3hs a left join $tb_online3hs_bogo b on a.num_mcode=b.num_mcode and a.num_serial=b.num_serial
...
$tpl->setLayout('sub');
if(debug()==true){
	$tpl->define("CONTENT","/html/online3hs/list_test.htm");
}else{
	$tpl->define("CONTENT","/html/online3hs/list.htm");
}
...
{
	GLOBAL $pageUtil,$BoardDB,$tb_online3hs,$tb_online3hs_bogo;
	...
	// 보고서 작성일자 
	$sql = "select TO_CHAR(dt_date, 'YYYY-MM-DD') dt_date from $tb_online3hs_bogo where num_oid='".$arr['num_oid']."' and num_mcode='".$arr['num_mcode']."' and num_serial='".$arr['num_serial']."'";
	...
	if($arr['str_phase2']=="Y"){
		...
		$sql_chk = "select count(*) as cnt from $tb_online3hs_bogo where num_oid='".$arr['num_oid']."' and num_mcode='".$arr['num_mcode']."' and num_serial='".$arr['num_serial']."'";
		$bogo_chk = $BoardDB->sqlFetchOne($sql_chk);
		if($bogo_chk == 1){
			$sql_bogo = "select str_bogo_acc3, str_bogo_acc2, str_bogo_acc1 from $tb_online3hs_bogo where num_oid='".$arr['num_oid']."' and num_mcode='".$arr['num_mcode']."' and num_serial='".$arr['num_serial']."'";
