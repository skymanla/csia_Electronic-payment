...
if($_SESSION[ADMIN]){//학교 관리자 올 결재(알바비점ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ)
	if($type=='p'){//학부모결재
		...
		$BoardDB->query($sql);
		$BoardDB->commit();
		WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "학부모 승인이 완료되었습니다.");
	}else if($type=='t'){
		$sql = "update $tb_online3hs set str_acc_tmp1='Y' where num_oid='$oid' and num_mcode='$code' and num_serial='$num'";
		$BoardDB->query($sql);
		$BoardDB->commit();
		WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "담임 승인이 완료되었습니다.");
	}
	/*else if($type=='c'){
		$sql = "update $tb_online3hs set str_acc_tmp2='Y' where num_oid='$oid' and num_mcode='$code' and num_serial='$num'";
		$BoardDB->query($sql);
		$BoardDB->commit();
		WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "담당부서 승인이 완료되었습니다.");
	}*/
	else if($type=='a'){
		...
		$BoardDB->query($sql);
		$BoardDB->commit();
		WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "교무부장 승인이 완료되었습니다.");
	}
}
...
if($_SESSION[acc_tmp1] == 7){//담당교직원
	...
	if($chk_acc[str_acc_tmp1] == "N"){
		WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "아직 이전 승인이 이루어지지 않았습니다.");
	}
	$sql = "update $tb_online3hs set str_acc_tmp2='Y' where num_oid='$oid' and num_mcode='$code' and num_serial='$num'";
	$BoardDB->query($sql);
	$BoardDB->commit();
	WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "승인이 완료되었습니다.");*/
	WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num", "잘못된 승인입니다.");
}else if($_SESSION[acc_tmp3] == 8){//교무부장
	//선 승인 방지용
