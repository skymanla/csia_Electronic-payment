...
switch ($REQUEST_METHOD) {
	case "GET":
		...
		//신청서 결재가 완료되지 않은 경우
		$sql = "select * from $tb_online3hs where num_oid='$oid' and num_mcode='$code' and num_serial='$num' $select_where";
		$chk = $BoardDB->sqlFetch($sql);
		
		if($chk['str_phase']!="Y"){
			WebApp::redirect("/?act=online3hs.read&mcode=$code&num=$num","아직 신청서 결재가 완료되지 않았습니다.");
		}
		//보고서 정보 가져오기
		$sql = "select * from $tb_online3hs_bogo where num_oid='$oid' and num_mcode='$code' and num_serial='$num' $select_where";
		$bogo = $BoardDB->sqlFetch($sql);
		if($bogo){
			WebApp::redirect("/?act=online3hs.modify2&num=$num&mcode=$code","이미 작성한 보고서가 있습니다.수정 페이지로 이동합니다.");
		}
		//신청서 정보 가져오기
		$sql = "SELECT 
		...
		//	$tpl->define("CONTENT","/html/online3hs/write2_test.htm");
		//}else{
			$tpl->define("CONTENT","/html/online3hs/write2.htm");
		//}
	...
	case "POST":
		...
		$sql = "insert into $tb_online3hs_bogo
				($insert_col)
				values
				($insert_val) ";
		//echo $sql; exit;
		...
		$updirpath = "files/online3hs/$str_year/$bid/$num";
