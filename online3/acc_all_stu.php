...
$data = $_REQUEST;
	...
	for($a=0;$a<count($data['chk_idx']);$a++){
		if($_SESSION[acc_tmp5]==98){
			$sql = "update $tb_online3 ".$default_col."='Y' ".$add_col." where num_oid='$oid' and num_mcode='".$data['mcode']."' and num_serial='".$data['chk_idx'][$a]."'";
			$BoardDB->query($sql);
			$BoardDB->commit();
		}else{
			$sql = "select ".$up_col." from $tb_online3 where num_oid='$oid' and num_mcode='".$data['mcode']."' and num_serial='".$data['chk_idx'][$a]."'";
