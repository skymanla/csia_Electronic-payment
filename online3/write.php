...
switch ($REQUEST_METHOD) {
	case "GET":
		...
		//print_r($_SESSION);
		$sql = "select * from $tb_online3 where num_oid='$oid' and num_mcode='$mcode' and str_id='".$_SESSION['USERID']."' and str_loginid='".$_SESSION['LOGONID']."' order by dt_date desc";
		...
		if($_SESSION['MEM_TYPE'][0] == s){
			...
			if ( !$_p_data['str_id_y'] && !$_SESSION['ADMIN']) {
				WebApp::redirect("/?act=online3.list&mcode=$mcode".$pagego,"자녀설정을 먼저 해주시기 바랍니다.");
				exit;
			}
		}
		...
		//남은 날짜 합산하기
		$sql = "select sum(str_date_tmp1) as sum_date from $tb_online3 where str_id='".$_SESSION['USERID']."' and str_loginid='".$_SESSION['LOGONID']."' and num_mcode='".$_GET['mcode']."' and num_oid='$oid' and str_year='$n_year'";
		...
		$tpl->define("CONTENT","/html/online3/write.htm");
		$tpl->parse("CONTENT");
	...
	case "POST":
		...
		//$str_year = date('Y');
		$max_serial = $BoardDB->sqlFetchOne("select max(num_serial) + 1 from $tb_online3 where num_oid=$oid and num_mcode=$code");
