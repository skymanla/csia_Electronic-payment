...
switch ($REQUEST_METHOD) {
	case "GET":
		...
		//print_r($_SESSION);
		$acc_link_val = "/?act=online3.acc_stu&mcode=$code&num=$num";
		$acc_title_val = "승인";
		if($_SESSION[ADMIN]){//관리자용 승인 및 반려 버튼
			...
			if($data[str_acc_tmp1] == "Y"){//학부모가 승인을 했으면 수정이 불가능하게 하자 -> 담임부터 수정 불가하도록
				...
				$tpl->assign(array("acc_link"=> "/?act=online3.modify&mcode=$code&num=$num", "acc_title"=>"수정"));
			}
		...
		if($_SESSION[acc_tmp2]==99 || $_SESSION[acc_tmp3]==8){//$_SESSION[acc_tmp1]==7 || 
			...
			$tpl->assign(array("back_url"=>"/?act=online3.giveback&mcode=$code&num=$num","back_title"=>"반려"));
		}
		//print_r($data);
		//exit;
		//신청날짜 생성
		...
		$tpl->setLayout('sub');
		$tpl->define("CONTENT","/html/online3/read.htm");
		$tpl->parse("CONTENT");
...
function _format_data(&$arr) {
	...
	if($_SESSION[ADMIN] || $_SESSION[acc_tmp1]==7 || $_SESSION[acc_tmp2]==99 || $_SESSION[acc_tmp3]==8 || $_SESSION[acc_tmp5]==98){
		$arr['deleteUrl'] = "/?act=online3.delete&mcode=".$mcode."&num=".$arr['num_serial']."&id=".$arr['str_id']."&id2=".$arr['str_loginid'];
	}else{
		$arr['deleteUrl'] = "/?act=online3.delete&mcode=".$mcode."&num=".$arr['num_serial'];
	}
