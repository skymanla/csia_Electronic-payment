...
$sql2 = "select * from $tb_online3_bogo where num_oid='$oid' and num_mcode='$mcode' and num_serial='$num' and str_id='$str_id' and str_loginid='$str_loginid'";
...
if($_SESSION[acc_tmp1]==7 || $_SESSION[acc_tmp2]==99 || $_SESSION[acc_tmp3]==8 || $_SESSION[ADMIN]){
	...
	if($chk[str_phase2]=="Y" || $chk2[str_bogo_acc2]=="Y"){
		WebApp::redirect("/?act=online3.read2&&num=$num&mcode=$mcode","승인이 되어 삭제가 불가능합니다.");
	}
}
...
$sql = "delete from $tb_online3_bogo where num_oid='$oid' and num_mcode='$mcode' and num_serial='$num' and str_id='$str_id' and str_loginid='$str_loginid'";
...
if($BoardDB->query($sql)){
	...
	//첨부파일 삭제
	$sql = "delete from $tb_online3_files where num_oid='$oid' and num_mcode='$mcode' and num_main='$num'";
	$BoardDB->query($sql);
	$BoardDB->commit();
